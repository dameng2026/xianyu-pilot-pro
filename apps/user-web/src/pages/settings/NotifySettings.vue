<template>
  <div class="notify-settings-shell">
    <div v-if="error" id="notify-settings-unavailable" class="global-notice error" role="alert">{{ error }}</div>
    ...
    <section class="notify-hero">
      ...
      <aside class="notify-health-card">
        ...
        <div class="notify-health-foot">
          <span>通知记录 {{ visualTodayCount }}</span>
          <span>当前页异常 {{ visualPendingCount }}</span>
    ...
    <fieldset
      class="notify-workspace-grid"
      :disabled="!settingsAvailable"
      aria-label="通知渠道与规则配置"
      :aria-describedby="error ? 'notify-settings-unavailable' : undefined"
    >
        ...
        <section v-if="selectedChannel" class="notify-panel notify-config-card">
          ...
          <div class="notify-panel-body notify-config-body">
            <div class="notify-status-strip" :class="{ disconnected: !isChannelConfigured(selectedChannel) }">
              ...
              <button type="button" class="notify-link-button" @click="test">
                重新测试
              </button>
            </div>
            ...
            <section v-if="selectedChannel.type === 'email'" class="notify-config-section">
              ...
              <div class="notify-form-grid two">
                ...
                <label class="notify-field">
                  <span>授权码</span>
                  <input v-model="selectedChannel.smtpPass" type="password" :placeholder="selectedChannel.smtpPassConfigured ? `已安全保存（尾号 ${selectedChannel.smtpPassLast4 || '****'}），留空表示保留` : '请输入 SMTP 授权码'" />
                </label>
                <label class="notify-field">
                  <span>发件人</span>
            ...
            <section v-else-if="selectedChannel.type === 'feishu_app'" class="notify-config-section">
              ...
              <div class="notify-form-grid two">
                ...
                <label class="notify-field">
                  ...
                  <input v-model="selectedChannel.secret" type="password" :placeholder="selectedChannel.secretConfigured ? `已安全保存（尾号 ${selectedChannel.secretLast4 || '****'}），留空表示保留` : '飞书自建应用 App Secret'" />
                </label>
                <label class="notify-field">
                  <span>Verification Token</span>
                  <input v-model="selectedChannel.verificationToken" type="password" :placeholder="selectedChannel.verificationTokenConfigured ? '已安全保存，留空表示保留' : '事件订阅的 Verification Token'" />
                </label>
                <label class="notify-field">
                  <span>Encrypt Key（可选）</span>
                  <input v-model="selectedChannel.encryptKey" type="password" :placeholder="selectedChannel.encryptKeyConfigured ? '已安全保存，留空表示保留' : '启用加密时填写'" />
                </label>
                <label class="notify-field">
              ...
              <div class="notify-tips">
                ...
                <p style="margin-top: 8px; color: #ff9800;">
                  <strong>提示：</strong>账号 Session 过期时，系统会通过此渠道主动推送通知。用户在飞书回复「二维码」即可获取登录二维码，扫码后系统自动恢复连接。
                </p>
              </div>
            </section>
            ...
            <section v-else-if="selectedChannel.type === 'pushplus'" class="notify-config-section">
              ...
              <div class="notify-form-grid two">
                <label class="notify-field notify-field-full">
                  ...
                  <input v-model="selectedChannel.receiver" type="password" :placeholder="selectedChannel.receiverConfigured ? `已安全保存（尾号 ${selectedChannel.receiverLast4 || '****'}），留空表示保留` : '请输入 PushPlus Token'" />
                </label>
                <label class="notify-field">
                  <span>超时时间（秒）</span>
            ...
            <section v-else class="notify-config-section">
                ...
                <label class="notify-field notify-field-full">
                  ...
                  <div class="notify-input-wrap">
                    <input
                      ...
                      type="text"
                      placeholder="https://notify.example.com/webhook/xianyu-assistant"
                    />
                    <button
                    ...
                    <small>仅支持 HTTPS；通用 Webhook 与 SMTP 域名还必须由管理员加入生产环境白名单。</small>
                  </label>
              </div>
              ...
              <div class="notify-form-grid three">
                <label class="notify-field">
                  <span>超时时间（秒）</span>
                  <input v-model.number="selectedChannel.timeoutSeconds" type="number" min="1" max="15" />
                </label>
                <label class="notify-field">
                  <span>重试次数</span>
                  <input v-model.number="selectedChannel.retryCount" type="number" min="0" max="10" />
              ...
              <div v-if="selectedChannel.type !== 'webhook'" class="notify-form-grid two">
                <label v-if="selectedChannel.type !== 'webhook'" class="notify-field">
                  ...
                  <input
                    ...
                    type="password"
                    :placeholder="selectedChannel.secretConfigured ? `已安全保存（尾号 ${selectedChannel.secretLast4 || '****'}），留空表示保留` : '可选，用于飞书 / 钉钉签名验证'"
                  />
                </label>
              </div>
            </section>
            ...
            <div class="notify-config-actions">
              <AppButton type="primary" :loading="saving" :disabled="!settingsAvailable" @click="save">保存当前页面配置</AppButton>
              <AppButton :loading="testing" :disabled="!settingsAvailable" @click="test">发送测试通知</AppButton>
            </div>
          </div>
        </section>
      ...
      <div class="notify-right-column">
          ...
          <div class="notify-rules-groups">
            ...
            >
              ...
              >
                ...
                <button
                  type="button"
                  class="notify-switch-button"
                  :aria-label="`${event.event}：${event.enabled ? '停用事件通知' : '启用事件通知'}`"
                  :aria-pressed="!!event.enabled"
                  @click="toggleEvent(event, 'enabled')"
                >
                  <ToggleSwitch :on="!!event.enabled" />
                </button>
                <button
                  type="button"
                  class="notify-switch-button"
                  :aria-label="`${event.event}：${event.app ? '关闭应用内通知' : '开启应用内通知'}`"
                  :aria-pressed="!!event.app"
                  @click="toggleEvent(event, 'app')"
                >
                  <ToggleSwitch :on="!!event.app" />
                </button>
              </div>
            </section>
          </div>
        ...
        <section class="notify-panel notify-logs-card">
          ...
          <div class="notify-panel-body notify-log-list">
            <div v-if="deliveryLogsLoadError" class="notify-truthful-empty error-state">
              通知日志暂时无法加载，请稍后刷新重试。
            </div>
            <div v-else-if="!loading && recentLogRows.length === 0" class="notify-truthful-empty">
              暂无真实投递记录。发送测试通知后，结果会显示在这里。
            </div>
            <article
        ...
        <section class="notify-preview-card">
          <div class="notify-preview-head">
            ...
            <span class="notify-preview-tag">真实通知</span>
          </div>

          <div class="notify-preview-phone">
            <div class="notify-phone-top"></div>
            <div class="notify-phone-screen">
              <div class="notify-phone-time">{{ previewTime }}</div>

              <div v-if="notificationsLoadError" class="notify-truthful-empty error-state">
                应用内通知暂时无法加载。
              </div>
              <div v-else-if="!loading && previewMessages.length === 0" class="notify-truthful-empty">
                暂无应用内通知
              </div>
          ...
          <div class="notify-foot-note">
            <span>仅展示接口返回的真实通知</span>
            <span>无数据时不会填充演示内容</span>
          </div>
        </section>
      </div>
    </fieldset>
...
const CHANNEL_LIBRARY = [
  ...
  },
  {
    ...
    icon: 'notifyFeishu',
    accent: 'cyan',
    description: '支持双向对话与扫码登录：账号 Session 过期时自动推送通知，回复「二维码」即可获取登录二维码，扫码后系统自动恢复连接。需在飞书开放平台创建自建应用。'
  },
  {
    key: 'dingtalk',
    type: 'dingtalk',
...
const CHANNEL_TUTORIALS = {
  webhook: {
    ...
    fields: [
      { label: 'Webhook 地址', detail: '必须是完整 http/https 地址，建议使用 https，避免被网关或浏览器策略拦截。' },
    ...
    checklist: [
      ...
      'Webhook 地址填的是公网或局域网内可访问的真实地址，不是浏览器页面地址。',
...
const notificationsLoadError = ref(false)
const deliveryLogsLoadError = ref(false)
...
const healthPercent = computed(() => {
  ...
  const liveRate = Number(((okCount / deliveryLogs.value.length) * 100).toFixed(1))
  return Math.max(0, Math.min(100, liveRate))
})

const heroHealthLabel = computed(() => deliveryLogs.value.length
  ? `${healthPercent.value.toFixed(1)}%（最近 ${deliveryLogs.value.length} 条）`
  : '暂无投递数据')
...
const visualTodayCount = computed(() => notificationTotal.value)
const visualPendingCount = computed(() => pendingCount.value)

const summaryCards = computed(() => [
  {
    label: '已启用渠道',
    value: String(channels.value.filter(channel => channel.enabled).length),
    detail: channels.value.some(channel => channel.enabled)
      ? channels.value.filter(channel => channel.enabled).map(channel => channel.name).join(' / ')
      : '尚未启用任何外部渠道',
    icon: 'notifyChannel',
    tone: 'blue'
  },
  {
    label: '已开启事件',
    value: String(enabledEventCount.value),
    detail: enabledEventCount.value ? '以当前已保存配置为准' : '尚未开启通知事件',
    icon: 'notifyEvent',
    tone: 'green'
  },
  {
    label: '通知记录数',
    value: String(visualTodayCount.value),
    detail: notificationsLoadError.value ? '通知记录暂时无法加载' : '以接口返回总数为准',
    icon: 'notifyClock',
    tone: 'indigo'
  },
  {
    label: '发送成功率',
    value: deliveryLogs.value.length ? `${healthPercent.value.toFixed(1)}%` : '—',
    detail: deliveryLogsLoadError.value ? '投递日志暂时无法加载' : '基于最近加载的投递记录',
    icon: 'notifyRate',
    tone: 'cyan'
  },
  {
    label: '待处理提醒',
    value: String(visualPendingCount.value),
    detail: visualPendingCount.value ? '失败投递与未读应用内通知合计' : '当前没有待处理提醒',
    icon: 'notifyAlert',
    tone: 'orange'
  }
])
...
const previewTime = computed(() => {
  const value = notifications.value[0]?.createdTime
  if (!value) return '--:--'
...
const selectedChannelStatus = computed(() => {
  ...
  if (!settingsAvailable.value) {
    return {
      title: '通知设置服务暂时不可用',
      description: '当前配置未成功加载，保存和测试已禁用，请刷新后重试。'
    }
  }
  ...
  return {
    title: `已填写 ${renderConnectionTarget(channel)} 的连接配置`,
    description: '配置字段已就绪，但仍需发送测试通知并在接收端确认真实送达。'
  }
})
...
function normalizeChannels(raw) {
  ...
  const map = new Map(list.map((item, index) => [
    String(item?.key || item?.type || item?.name || `custom-${index + 1}`),
    item
  ]))
...
function normalizeChannelRecord(item, meta) {
  ...
  return {
    ...
    receiverConfigured: toBool(item.receiverConfigured, false),
    receiverLast4: String(item.receiverLast4 || ''),
    secretConfigured: toBool(item.secretConfigured, false),
    ...
    verificationTokenConfigured: toBool(item.verificationTokenConfigured, false),
    verificationTokenLast4: String(item.verificationTokenLast4 || ''),
    encryptKey: String(item.encryptKey || item.encrypt_key || ''),
    encryptKeyConfigured: toBool(item.encryptKeyConfigured, false),
    encryptKeyLast4: String(item.encryptKeyLast4 || ''),
...
function isChannelConfigured(channel) {
  ...
  if (channel.type === 'pushplus') {
    return Boolean(channel.receiver || channel.receiverConfigured)
  }
  ...
  return /^https:\/\//i.test(String(channel.webhookUrl || ''))
}

function channelStatusText(channel) {
  return channel.enabled ? '已启用' : '未启用'
}
...
function formatCost(value) {
  ...
  if (!Number.isFinite(ms) || ms <= 0) return '1.2s'
...
function serializeChannels() {
  return channels.value.map(channel => ({
    ...
    receiverConfigured: !!channel.receiverConfigured,
    ...
    encryptKeyConfigured: !!channel.encryptKeyConfigured,
...
async function load(showErrorNotice = false) {
  ...
  deliveryLogsLoadError.value = false
  ...
  if (noticeRes.status === 'fulfilled') {
    ...
    notificationTotal.value = Number(noticeRes.value?.data?.total ?? notifications.value.length) || 0
  } else {
    notifications.value = []
    notificationTotal.value = 0
    notificationsLoadError.value = true
  }
...
async function save() {
  ...
  clearNotices()
  const invalidEnabledChannel = channels.value.find(channel => channel.enabled && !isChannelConfigured(channel))
  if (invalidEnabledChannel) {
...
async function test() {
  ...
  try {
    const res = await testNotification({
      title: '通知渠道测试',
      content: `来自 ${selectedChannel.value?.name || '通知中心'} 的连接测试`
    })
    ...
    error.value = err.message || '测试发送失败'
  } finally {
    testing.value = false
  }
}
...
.notify-truthful-empty.error-state {
  border-color: #f0c4c4;
  color: #b84f4f;
  background: #fff8f8;
}
...
/* 桌面工作台在手机上仍保留信息层级，但避免 Hero 占满首屏。 */
