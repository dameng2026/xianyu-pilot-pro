...
const beginnerSteps = [
  ...
  { title: '保持 WebSocket 在线', desc: '进入「连接管理」页面，确认账号的 WS 状态显示为"在线"。WS 在线是接收消息和触发自动化的前提。', route: 'connections', routeName: '连接管理' },
  ...
  },
  {
    ...
    modules: [
      { name: '闲鱼账号', ico: '👤', desc: '所有店铺账号的统一管理中心。', route: 'accounts', shot: 'accounts.png', fig: '图 6.3 / 闲鱼账号管理。', features: [
        ...
        '<strong>快捷操作抽屉</strong>：编辑 Cookie、刷新资料、同步商品、跳转自动回复/发货、连接管理、在线消息、登录验证、重新扫码、人脸验证、自动评价、账号密码登录、统一配置。'
      ]},
      { name: '连接管理', ico: '🔗', desc: '监控所有账号的 WebSocket 长连接状态。', route: 'connections', shot: 'connections.png', fig: '图 6.4 / 连接管理。', features: [
        '<strong>统计卡片</strong>：在线 / 离线 / 重连中 / Cookie 异常等数量。',
        '<strong>连接详情</strong>：每条连接的 Cookie 状态、WS 状态、心跳、延迟、自动重连、代理。',
        '<strong>操作</strong>：断开、启动、刷新 Cookie、检查登录。',
        '<strong>默认重连策略</strong>：最大重试 3 次，重试间隔 2 秒，自动重连开关。'
      ]},
...
// FAQ
const faqItems = [
  { q: '为什么我的账号显示 WS 离线？', a: '通常是 Cookie 过期或触发闲鱼滑块风控。进入「连接管理」点击"刷新 Cookie"或"检查登录"，必要时到「闲鱼账号」重新扫码授权。如果触发滑块，系统会自动求解，可在「滑块求解」查看结果。' },
  { q: '自动发货没触发，订单一直是"待处理"怎么办？', a: '按以下顺序排查：① WS 是否在线（连接管理）；② 商品是否配置了对应时机的发货规则（自动发货）；③ 卡密库存或货源是否充足（卡密仓库/货源库）；④ 在「发货记录」看失败原因。常见原因：缺货、配置错误、WS 断开。' },
