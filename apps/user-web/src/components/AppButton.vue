<template>
  <button :class="['app-btn', type || 'ghost', { loading }]" :disabled="disabled || loading">
    <span v-if="loading" class="app-btn-spinner" aria-hidden="true"></span>
    <slot>{{ loading ? loadingText : '' }}</slot>
  </button>
</template>
<script setup>
defineProps({ type: String, disabled: Boolean, loading: Boolean, loadingText: { type: String, default: '处理中...' } })
</script>
<style scoped>
