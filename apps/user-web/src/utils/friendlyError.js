...
export function friendlyError(err, fallback = '操作失败，请稍后重试') {
  ...
  const requestId = err && typeof err === 'object'
    ? err.requestId
    : raw.match(/错误编号：([^）\s]+)/)?.[1]
  ...
  if (/后台任务可能仍在运行|历史记录|可稍后恢复/i.test(raw)) return withRequestId(raw)

  // 1. 精确匹配闲鱼错误码
  const trimmed = raw.trim()
  if (XIANYU_ERROR_MAP[trimmed]) return withRequestId(XIANYU_ERROR_MAP[trimmed])

  // 2. 模糊匹配关键词
  for (const { pattern, message } of KEYWORD_PATTERNS) {
    if (pattern.test(raw)) return withRequestId(message)
  }

  // 3. 包含闲鱼错误码（如 "FAIL_SYS_USER_VALIDATE: xxx"）
  for (const code of Object.keys(XIANYU_ERROR_MAP)) {
    if (raw.includes(code)) return withRequestId(XIANYU_ERROR_MAP[code])
  }
