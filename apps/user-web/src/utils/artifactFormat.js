...
export function formatStateVariable(v) {
  ...
  if (typeof value === 'string' && value.trim().startsWith('{')) {
    try { value = JSON.parse(value) } catch {
      // Keep the original string when it only resembles JSON.
    }
  }
  if (typeof value === 'object' && value !== null) {
    try { value = JSON.stringify(value, null, 2) } catch {
      // Circular values are rendered through their default string conversion.
    }
  }
  return `【${type}】 ${name}\n${value}`
}
