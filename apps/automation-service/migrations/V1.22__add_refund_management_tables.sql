...
CREATE TABLE IF NOT EXISTS `xianyu_refund` (
    ...
    `logistics_mail_no` VARCHAR(128) NULL COMMENT '物流单号（commonData.mailNo）',
...
-- 退款同步任务追踪表：记录每次退款同步任务的状态（参考 xianyu_goods_sync_task 模式）
...
-- 账号级退款同步状态表：记录每个账号的最后同步时间、缓存过期判断、任务去重
