...
-- 注意：MySQL 8.4 不支持 ADD COLUMN IF NOT EXISTS 语法，使用独立 ALTER TABLE 语句。
...
ALTER TABLE `xianyu_captcha_solve_record`
  ADD COLUMN `failure_reason` VARCHAR(64) NOT NULL DEFAULT ''
...
ALTER TABLE `xianyu_captcha_solve_record`
  ADD INDEX `idx_csr_priority_status_queued` (`status`, `priority`, `queued_at`);

-- 超时清理索引：用于定时扫描"进行中但超时"的记录
ALTER TABLE `xianyu_captcha_solve_record`
  ADD INDEX `idx_csr_status_started_at` (`status`, `started_at`);
