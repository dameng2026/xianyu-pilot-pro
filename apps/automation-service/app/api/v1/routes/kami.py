...
async def list_kami_configs(
    ...
    try:
        ...
        result = await db.execute(
            select(CardGroup).where(CardGroup.tenant_id == tenant_id, CardGroup.deleted == 0)
            .order_by(CardGroup.id.desc())
        )
        configs = result.scalars().all()
        return ResultObject.success([_group_to_dto(c) for c in configs])
    except Exception as e:
        return safe_route_failure(logger, e, operation="list card configs", user_message="获取卡密配置失败，请稍后重试")
...
async def save_kami_config(
    ...
    try:
        ...
        return safe_route_failure(logger, e, operation="save card config", user_message="保存卡密配置失败，请稍后重试")
...
async def list_kami_items(
    ...
    try:
        ...
        return safe_route_failure(logger, e, operation="list card items", user_message="获取卡密列表失败，请稍后重试")
...
async def add_kami_item(
    ...
    try:
        ...
        return safe_route_failure(logger, e, operation="add card item", user_message="添加卡密失败，请稍后重试")
...
async def batch_import_kami(
    ...
    try:
        ...
        return safe_route_failure(logger, e, operation="batch import card items", user_message="批量导入卡密失败，请稍后重试")
