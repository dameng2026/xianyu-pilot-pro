...
def _tenant_account_from_request(
    request: Request,
    current_user: dict,
) -> tuple[int, int | None]:
    """从请求中提取 tenant_id 和 account_id。

    tenant_id 只来自已认证身份，禁止从可伪造请求头读取或回退到租户 1。
    account_id 可空（表示全租户通用模板）
    """
...
async def list_templates(
    ...
    tenant_id, account_id = _tenant_account_from_request(request, current_user)
...
async def save_template(
    ...
    result = await db.execute(
        text("""
            INSERT INTO quick_reply_template (tenant_id, account_id, title, content, sort_order, status, deleted, created_time, updated_time)
            VALUES (:tenant_id, :account_id, :title, :content, :sort_order, 1, 0, NOW(), NOW())
        """),
        {
            "tenant_id": tenant_id,
            "account_id": account_id,
...
async def delete_template(
    ...
    """软删除快捷回复模板。"""
    tenant_id, _ = _tenant_account_from_request(request, current_user)
    result = await db.execute(
        text("""
...
async def init_default_templates(
    ...
    check = await db.execute(
        text("SELECT COUNT(*) AS cnt FROM quick_reply_template WHERE tenant_id = :tenant_id AND deleted = 0"),
        {"tenant_id": tenant_id}
    )
