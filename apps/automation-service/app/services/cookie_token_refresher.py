...
from .notify_dispatcher import (
    clear_all_account_status_notifications,
    ...
    async with _states_lock:
        for acc in accounts:
            ...
            if aid not in _states:
                ...
                # 更新 tenant_id（可能变化）
                _states[aid].tenant_id = int(acc["tenant_id"])
...
async def _update_cookie_and_token_in_db(
    ...
    try:
        ...
        log_service_failure(
            logger, e, operation="persist_refreshed_credentials",
...
async def _update_cookie_status(
    ...
    try:
        ...
        log_service_failure(
            logger, e, operation="update_cookie_status",
            tenant_id=tenant_id, account_id=account_id, level=logging.WARNING,
        )
...
def _notify_ws_client_credentials_updated(
    ...
    try:
        ...
        log_service_failure(
            logger, e, operation="notify_ws_credentials_updated",
            account_id=account_id, level=logging.DEBUG,
        )
...
async def _do_cookie_keepalive(state: AccountRefreshState) -> bool:
    ...
    try:
        ...
        if result.get("success"):
            ...
            try:
                await _update_cookie_status(
                    state.account_id, state.tenant_id, 1,
                    "OK", "Cookie 保活成功",
                )
            except Exception:
            ...
            log_service_failure(
                logger, RuntimeError(), operation="cookie_keepalive",
            ...
            if "CAPTCHA_NEEDED" in err_str or "FAIL_SYS_USER_VALIDATE" in err_str or "RGV587" in err_str:
                ...
                try:
                    from .ws_client import ws_manager
                    await ws_manager.stop_client(state.account_id)
                    logger.info("Cookie Session 过期，已断开 WS 连接 accountId=%d", state.account_id)
                except Exception as ws_err:
                    log_service_failure(
                        logger, ws_err, operation="stop_ws_after_session_expiry",
                        tenant_id=state.tenant_id, account_id=state.account_id, level=logging.WARNING,
                    )
            return False
        ...
        log_service_failure(
            logger, e, operation="cookie_keepalive",
            tenant_id=state.tenant_id, account_id=state.account_id,
        )
        state.last_cookie_keepalive_ok = False
        state.last_error = "Cookie 保活异常，请稍后重试"
        return False
...
async def _call_has_login(account_id: int, tenant_id: int) -> dict:
    ...
    try:
        ...
        log_service_failure(
            logger, e, operation="call_has_login",
            tenant_id=tenant_id, account_id=account_id,
        )
        return {"success": False, "errorCode": "HAS_LOGIN_UNAVAILABLE", "error": "Cookie 保活服务暂时不可用"}
    ...
    if any(kw in body_text for kw in risk_keywords):
        return {"success": False, "errorCode": "CAPTCHA_NEEDED", "error": "Cookie 保活触发人机验证"}
    ...
    login_confirmed = resp.status_code == 200 and _is_has_login_confirmed(body_text)
    if cookie_changed and login_confirmed:
        try:
            ...
            async with async_session() as db:
                await db.execute(
                    ...
                    """),
                    {"cookie": encrypted, "account_id": account_id, "tenant_id": tenant_id},
                )
                await db.commit()
            ...
            log_service_failure(
                logger, e, operation="persist_has_login_cookie",
                tenant_id=tenant_id, account_id=account_id, level=logging.WARNING,
            )

    # 8. 仅结构化响应明确确认登录成功时才算保活成功。
    if login_confirmed:
    ...
    if resp.status_code == 200:
        return {
            ...
            "error": "hasLogin 响应未明确确认登录状态",
        }
    return {"success": False, "errorCode": "HAS_LOGIN_REJECTED", "error": "Cookie 保活请求未被平台接受"}
...
async def _do_mh5tk_refresh(state: AccountRefreshState, cookie_str: str) -> bool:
    ...
    try:
        ...
        if new_cookie and new_m_h5_tk:
            logger.info(
                "_m_h5_tk 刷新成功 accountId=%d, new_token_len=%d",
                state.account_id, len(new_m_h5_tk),
            )
            # 写回 DB
            await _update_cookie_and_token_in_db(
                state.account_id, state.tenant_id,
                cookie_str=new_cookie, m_h5_tk=new_m_h5_tk,
            )
            # 通知 WS 客户端
        ...
        log_service_failure(
            logger, e, operation="refresh_m_h5_tk",
            tenant_id=state.tenant_id, account_id=state.account_id,
        )
        state.last_mh5tk_refresh_ok = False
        state.last_error = "_m_h5_tk 刷新异常，请稍后重试"
        return False
...
async def _do_ws_token_refresh(state: AccountRefreshState, cookie_str: str, m_h5_tk: str) -> bool:
    ...
    try:
        ...
        if access_token:
            logger.info(
                "WS Token 刷新成功 accountId=%d, token_len=%d",
                state.account_id, len(access_token),
            )
            # 如果刷新过程中 cookie 也被更新了，写回 DB
            if refreshed_cookie and refreshed_cookie != cookie_str:
                await _update_cookie_and_token_in_db(
                    state.account_id, state.tenant_id,
                    cookie_str=refreshed_cookie, m_h5_tk=effective_m_h5_tk,
                )
                ...
                await _update_cookie_and_token_in_db(
                    state.account_id, state.tenant_id, m_h5_tk=effective_m_h5_tk,
                )
        ...
        log_service_failure(
            logger, e, operation="refresh_ws_token",
            tenant_id=state.tenant_id, account_id=state.account_id,
        )
        state.last_ws_token_refresh_ok = False
        state.last_error = "WS Token 刷新异常，请稍后重试"
        return False
...
async def _dispatcher_loop() -> None:
    ...
    while _dispatcher_running:
        try:
                    ...
                    try:
                        async with async_session() as db:
                            row = (await db.execute(
                                ...
                                {"aid": state.account_id, "tid": state.tenant_id},
                            )).mappings().first()
                    except Exception as e:
                        log_service_failure(
                            logger, e, operation="load_refresh_credentials",
                            tenant_id=state.tenant_id, account_id=state.account_id, level=logging.WARNING,
                        )
                        continue
            ...
            log_service_failure(logger, e, operation="cookie_token_dispatcher")
...
async def force_refresh_account(account_id: int, tenant_id: int, refresh_type: str = "all") -> dict:
    """手动触发单账号刷新（供内部 API 调用）
    refresh_type: all / cookie / mh5tk / ws_token
    """
    try:
        scoped_account_id = int(account_id)
        scoped_tenant_id = int(tenant_id)
    except (TypeError, ValueError):
        return {"success": False, "errorCode": "INVALID_SCOPE", "error": "账号或租户参数无效"}
    if scoped_account_id <= 0 or scoped_tenant_id <= 0:
        return {"success": False, "errorCode": "INVALID_SCOPE", "error": "账号或租户参数无效"}

    async with _states_lock:
        state = _states.get(scoped_account_id)
    if state is not None and state.tenant_id != scoped_tenant_id:
        return {"success": False, "errorCode": "ACCOUNT_NOT_FOUND", "error": "账号不存在"}
    ...
    if state is None:
        ...
        try:
            async with async_session() as db:
                row = (await db.execute(
                    text(
                        "SELECT a.id, a.tenant_id, auth.cookie_status FROM xianyu_account a "
                        "JOIN xianyu_account_auth auth "
                        "  ON auth.account_id = a.id AND auth.tenant_id = a.tenant_id "
                        " AND auth.deleted = 0 "
                        " AND auth.id = ("
                        ...
                        "   LIMIT 1"
                        " ) "
                        "WHERE a.id = :aid AND a.tenant_id = :tid AND a.deleted = 0"
                    ),
                    {"aid": scoped_account_id, "tid": scoped_tenant_id},
                )).mappings().first()
            if not row:
                return {"success": False, "errorCode": "ACCOUNT_NOT_FOUND", "error": "账号不存在"}
            state = AccountRefreshState(
            ...
            async with _states_lock:
                existing = _states.get(scoped_account_id)
                if existing is not None and existing.tenant_id != scoped_tenant_id:
                    return {"success": False, "errorCode": "ACCOUNT_NOT_FOUND", "error": "账号不存在"}
                _states[scoped_account_id] = state
        except Exception as e:
            log_service_failure(
                logger, e, operation="load_manual_refresh_account",
                tenant_id=scoped_tenant_id, account_id=scoped_account_id,
            )
            return {"success": False, "errorCode": "REFRESH_UNAVAILABLE", "error": "账号刷新服务暂时不可用"}
    ...
    async with state.lock:
        ...
        try:
            ...
            log_service_failure(
                logger, e, operation="load_manual_refresh_credentials",
                tenant_id=scoped_tenant_id, account_id=scoped_account_id,
            )
            return {"success": False, "errorCode": "REFRESH_UNAVAILABLE", "error": "账号刷新服务暂时不可用"}

        if not row:
            return {"success": False, "error": "账号凭据不存在"}
