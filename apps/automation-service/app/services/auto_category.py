...
from ..services.category_data import merge_candidates
...
def upload_image_to_xianyu(
    ...
    if not cdn_url:
        logger.error(
            "上传图片后未能提取到 CDN URL responseType=%s responseKeys=%s",
            type(result).__name__,
            sorted(str(key) for key in result.keys())[:20] if isinstance(result, dict) else [],
        )
        raise RuntimeError("UPLOAD_NO_CDN_URL")

    logger.info("图片上传成功 CDN URL已提取 尺寸=%dx%d", width, height)
...
def call_category_recommend(
    ...
    logger.info(
        "正在调用分类推荐接口 appKey=%s cdnUrlPresent=%s 图片尺寸=%dx%d",
        MTOP_APP_KEY,
        bool(cdn_url),
        width,
        height,
    ...
    if "SIGN_FAILED" in ret_msg or "ILLEGAL_SIGN" in ret_msg:
        logger.error("分类接口签名失败, 请检查 appKey 是否已变更, appKey=%s, ret=%s", MTOP_APP_KEY, ret_msg)
        raise RuntimeError("CATEGORY_SIGN_FAILED")

    if "SUCCESS" not in ret_msg:
        logger.error("分类接口调用失败, ret=%s", ret_msg)
        raise RuntimeError(f"CATEGORY_API_FAILED: {ret_msg}")

    logger.info("分类推荐接口调用成功, ret=%s", ret_msg)
    return result
...
def select_best_category(
    ...
    # 用于 merge_candidates 在本地分类树中构建完整父级链路
    candidates = []
...
def auto_category(
    ...
    try:
        ...
        error_msg = str(e)
        logger.error("步骤1-图片上传CDN失败: %s", error_msg)
        return _build_response(
    ...
    try:
        ...
        error_msg = str(e)
        logger.error("步骤2-分类推荐接口调用失败: %s", error_msg)
        return _build_response(
            success=False, source="local_category", auto_selected=False,
            fallback_required=True, fallback_reason=error_msg,
    ...
    try:
        ...
        # 将候选分类同步到分类树中（非关键操作，失败不影响主流程）
        try:
            candidates = selection.get("candidates", [])
            if candidates:
                added = merge_candidates(candidates)
                if added > 0:
                    logger.info("分类树更新: 新增 %d 个分类", added)
        except Exception as e:
            logger.warning(
                "分类树同步失败（不影响自动分类结果） errorType=%s",
                type(e).__name__,
            )
    except Exception as e:
        error_msg = str(e)
        logger.error("步骤3-分类选择异常: %s", error_msg)
        return _build_response(
            success=False, source="manual", auto_selected=False,
            fallback_required=True, fallback_reason=error_msg,
            cdn_image_url=cdn_url,
