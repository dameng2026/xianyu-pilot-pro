...
for (const forbiddenChunk of ['art-chat-window-', 'art-fireworks-effect-']) {
  assert.equal(
    files.some((file) => path.basename(file).startsWith(forbiddenChunk)),
    false,
    `disabled demo feature must not ship: ${forbiddenChunk}`,
  )
}
