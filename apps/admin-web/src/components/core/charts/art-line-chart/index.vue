...
<script setup lang="ts">
  ...
  const normalizedMaxValue = computed(() => {
    if (typeof props.yAxisMax === 'number') {
      return props.yAxisMax
    }

    return maxValue.value > 0 ? maxValue.value : 1
  })

  const normalizedMinValue = computed(() => {
    if (typeof props.yAxisMin === 'number') {
  ...
  const createSeriesItem = (config: {
    ...
    return {
      ...
      lineStyle: {
        ...
        cap: 'round' as const,
        join: 'round' as const,
        opacity: config.lineStyleType === 'dashed' ? 0.88 : 1,
      ...
      itemStyle: {
        color: config.color,
        borderColor: '#ffffff',
        borderWidth: (config.symbol ?? props.symbol) === 'none' ? 0 : 2,
        shadowBlur: config.shadowBlur ? Math.max(6, Math.round(config.shadowBlur * 0.6)) : 0,
      ...
      emphasis: {
        ...
        lineStyle: {
          ...
          shadowBlur: Math.max(config.shadowBlur || 0, 8),
          shadowColor: config.shadowColor || config.color,
          shadowOffsetY: config.shadowOffsetY || 0
        }
      },
  ...
  const generateChartOptions = (isInitial = false): EChartsOption => {
    const options: EChartsOption = {
      ...
      grid: getGridWithLegend(props.showLegend && isMultipleData.value, props.legendPosition, {
        top: props.gridPadding?.top ?? 15,
        right: props.gridPadding?.right ?? 15,
        bottom: props.gridPadding?.bottom,
        left: props.gridPadding?.left ?? 0
      }),
      ...
      xAxis: {
        ...
        axisLabel: getAxisLabelStyle(props.showXAxisLabel ?? props.showAxisLabel)
      },
      yAxis: {
        type: 'value',
        min: normalizedMinValue.value,
        max: normalizedMaxValue.value,
        splitNumber: props.splitNumber,
        axisLabel: getAxisLabelStyle(props.showYAxisLabel ?? props.showAxisLabel),
        axisLine: getAxisLineStyle(props.showAxisLine),
    ...
    if (isMultipleData.value) {
      ...
      options.series = multiData.map((item, index) => {
        ...
        return createSeriesItem({
          ...
          areaStyle,
          lineStyleType: item.lineStyleType,
