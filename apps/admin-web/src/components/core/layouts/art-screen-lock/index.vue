        ...
        <div class="flex-c flex-col">
          ...
          >
            ...
            <ElAlert
              ...
              type="info"
              :closable="false"
              title="仅用于离席时遮挡当前页面；处理敏感操作前请退出登录。"
            />
...
<script setup lang="ts">
  ...
  import { createLockVerifier, verifyLockPassword } from '@/utils/lock-verifier'
  ...
  const disableDevTools = () => {
    // 浏览器端无法建立真正的调试器安全边界；拦截 F12、刷新、复制和切换窗口只会损害可用性。
    // 保留清理函数契约，但企业版不安装这些侵入式键盘/鼠标拦截器。
    const enableIntrusiveBrowserBlocking = false
    if (!enableIntrusiveBrowserBlocking) return () => undefined
  ...
  const handleLock = async () => {
    ...
    try {
      ...
      const verifier = await createLockVerifier(formData.password)
      userStore.setLockPassword(verifier)
      userStore.setLockStatus(true)
      visible.value = false
      formData.password = ''
    } catch (error) {
      if (error instanceof Error) {
        ElMessage.error('无法安全创建锁屏校验值，请退出登录后重试')
      }
    }
  }
