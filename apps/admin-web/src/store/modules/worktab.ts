    ...
    const openTab = (tab: WorkTab): void => {
      ...
      if (existingIndex === -1) {
        ...
        if (tab.fixedTab) {
          ...
          // 限制非固定标签页数量，避免持久化数据和 DOM 节点过多
          // 超过 15 个时移除最早的非固定标签页
          const nonFixedTabs = opened.value.filter((t) => !t.fixedTab)
          if (nonFixedTabs.length > 15) {
            const oldestNonFixed = nonFixedTabs[0]
            const oldestIndex = opened.value.findIndex((t) => t === oldestNonFixed)
            if (oldestIndex !== -1) {
              opened.value.splice(oldestIndex, 1)
            }
          }
        }
