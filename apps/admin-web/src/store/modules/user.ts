...
import { revokeAdminSession } from '@/api/auth'
...
export const useUserStore = defineStore(
  ...
  () => {
    ...
    /**
     * 设置搜索历史（最多保留 20 条，避免持久化数据过大）
     * @param list 搜索历史列表
     */
    const setSearchHistory = (list: AppRouteRecord[]) => {
      searchHistory.value = Array.isArray(list) ? list.slice(0, 20) : []
    }
    ...
    const setToken = (newAccessToken: string, newRefreshToken?: string) => {
      ...
      // 后端当前仅签发访问令牌。新会话未返回刷新令牌时必须清除历史值，
      // 避免旧版本持久化的“伪刷新令牌”被后续逻辑误用。
    ...
    const logOut = () => {
      ...
      if (mediaToken) {
        void revokeAdminSession(mediaToken).catch(() => {
          ElMessage.warning('本地已退出，但服务端会话撤销未确认；如非本人操作，请立即联系管理员')
        })
      }
      ...
      // 清空 HTTP 请求缓存（用户信息、菜单等），避免下个用户看到上个用户的数据
      clearHttpCache()
      // 重置路由状态
