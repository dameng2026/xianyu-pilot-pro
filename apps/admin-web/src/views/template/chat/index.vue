    ...
    <div class="box-border flex-1 h-full max-md:h-[calc(70%-30px)]">
      <div class="flex-cb pt-4 px-4 pb-0 mb-5">
        ...
        <div class="flex-c gap-2">
          <ArtIconButton icon="ri:phone-line" label="发起语音通话" circle class="size-11 text-g-600" />
          <ArtIconButton icon="ri:video-on-line" label="发起视频通话" circle class="size-11 text-g-600" />
          <ArtIconButton icon="ri:more-2-fill" label="打开更多操作" circle class="size-11 text-g-600" />
        </div>
      </div>
...
<script setup lang="ts">
  ...
  const userName = computed(() => userStore.getUserInfo.userName || '我')
