...
<template>
  <div class="menu-page art-full-height">
    ...
    <ElCard class="art-table-card">
      ...
      >
        <template #left>
          <ElButton v-auth="'add'" :disabled="!canMutate" @click="handleAddMenu" v-ripple> 添加菜单 </ElButton>
          <ElButton :disabled="listState !== 'ready'" @click="toggleExpand" v-ripple>
            {{ isExpanded ? '收起' : '展开' }}
          </ElButton>
        </template>
      </ArtTableHeader>
      ...
      <AdminDataState
        ...
        state="error"
        title="菜单配置暂时不可用"
        description="无法确认线上菜单，新增、编辑和删除已暂停。"
        @retry="getMenuList"
      />
      <AdminDataState
...
<script setup lang="ts">
  ...
  const { columnChecks, columns } = useTableColumns(() => [
    ...
    { prop: 'date', label: '编辑时间', formatter: () => '-' },
    {
      prop: 'status',
      label: '状态',
  ...
  const handleSubmit = async (formData: MenuFormData): Promise<void> => {
    if (!canMutate.value) {
      ElMessage.warning('菜单配置尚未成功读取，当前不能保存')
      return
    }
  ...
  const handleDeleteMenu = async (row: AppRouteRecord): Promise<void> => {
    if (listState.value === 'error' || !row.id) {
      ElMessage.error('菜单标识缺失或列表不可用，无法安全删除')
      return
    }
    try {
  ...
  const handleDeleteAuth = async (row: AppRouteRecord): Promise<void> => {
    if (listState.value === 'error' || !row.id) {
      ElMessage.error('权限标识缺失或列表不可用，无法安全删除')
      return
    }
    try {
