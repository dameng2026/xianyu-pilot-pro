  ...
  <div class="feature-switch-page">
    <ElCard shadow="never" class="filter-card">
      <div class="page-title-row">
        <div>
          <h2>功能管理</h2>
          <p>按账号等级控制前台各功能页面的访问开关。第一行为「店铺数量」：填写各等级可绑定的闲鱼店铺数，0 表示无限制（普通用户 1 / VIP 单店版 1 / VIP 不限 / SVIP 不限）。后续功能行分别控制普通用户、VIP（单店版）、VIP、SVP 四档的独立开关；VIP（单店版）与 VIP 功能权限保持一致，随 VIP 开关联动。"全部"为快捷开关：开启时三列全部打开，关闭时三列全部关闭。"维护"列开启后，所有用户进入该页面都会弹窗提示"正在维护升级中"，优先级高于一切。"限制模式"列为单选，只作用于<strong>等级开关未开启</strong>的用户，等级开关已开启的用户不受影响、正常使用：无限制（等级不足时按等级关系引导升级）/ 预览模式（等级不足的用户可进入页面查看数据，但不可执行业务操作如运行/发送）/ 不可进入（等级不足的用户直接无法访问该页面）。判定优先级：维护 &gt; 等级开关 &gt; 限制模式（预览/不可进入）。</p>
        </div>
