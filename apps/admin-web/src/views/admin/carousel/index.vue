  ...
  <div class="admin-page">
    <ElCard shadow="never" class="hero-card">
      <div class="page-title-row">
        <div>
          <h2>首页轮播配置</h2>
          <p>直接在当前页面维护前台首页轮播图，支持多图排序、上传、跳转链接和启停控制</p>
        </div>
        <div class="toolbar-actions">
          <ElButton :disabled="loading || saving" @click="loadConfig">恢复当前配置</ElButton>
          <ElButton
            type="primary"
            :loading="saving"
      ...
      <div v-if="form.legacyItemCount > 1" class="migration-tip">
        检测到旧版存在 {{ form.legacyItemCount }} 条轮播位记录，保存后会自动合并为一组前台轮播配置。
      </div>
    </ElCard>
      ...
      <ElCard shadow="never" class="preview-card">
        ...
        <div v-if="activePreview" class="preview-stage">
          <div class="preview-main">
            <img class="preview-image" :src="resolveImage(activePreview.imageUrl)" :alt="activePreview.title || form.title || '首页轮播'" />
            <div class="preview-overlay">
              <strong>{{ activePreview.title || form.title || '首页轮播' }}</strong>
              <p>{{ activePreview.description || form.description || '前台首页顶部轮播图' }}</p>
            </div>
          </div>
        ...
        <ElForm ref="formRef" :model="form" :rules="rules" label-width="110px" label-position="right" v-loading="loading">
          <div class="form-grid">
            <ElFormItem label="轮播标题" prop="title">
              <ElInput v-model="form.title" placeholder="例如：首页活动轮播" maxlength="60" show-word-limit />
            </ElFormItem>
            ...
            <div class="cover-editor">
              <div class="cover-editor-head">
                <div>
                  ...
                  <span>前台会按照下方顺序逐张轮播，第一张可用图片将作为主图和主链接</span>
                </div>
...
defineOptions({ name: 'AdminCarouselPage' })
...
async function handleSave() {
  if (configState.value !== 'ready') {
    ElMessage.warning('轮播配置尚未成功读取，当前不能保存')
    return
  }
  ...
  saving.value = true
  try {
    let saved: CarouselItem
    if (form.id) {
      saved = await updateCarousel(savePlan.payload)
    } else {
      saved = await saveCarousel(savePlan.payload)
    }

    if (savePlan.deleteIds.length > 0) {
      await Promise.all(savePlan.deleteIds.map(id => deleteCarousel(id)))
    }
...
async function handleImageUpload(index: number, options: UploadRequestOptions) {
  if (configState.value !== 'ready') {
    ElMessage.warning('轮播配置尚未成功读取，当前不能上传图片')
    return
  }
  try {
...
async function handleImportFromUrl(index: number) {
  if (configState.value !== 'ready') {
    ElMessage.warning('轮播配置尚未成功读取，当前不能导入图片')
    return
  }
