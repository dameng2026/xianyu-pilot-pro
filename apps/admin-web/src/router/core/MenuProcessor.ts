...
export class MenuProcessor {
  /** 菜单缓存（同会话内复用，避免每次路由守卫都重新过滤和规范化） */
  private cachedMenuList: AppRouteRecord[] | null = null
  /** 缓存对应的角色签名，角色变更时重新计算 */
  private cachedRolesSignature = ''
  ...
  async getMenuList(): Promise<AppRouteRecord[]> {
    ...
    const rolesSignature = (userStore.info?.roles || []).join(',')

    // 命中缓存：同会话且角色未变时直接返回
    if (this.cachedMenuList && this.cachedRolesSignature === rolesSignature) {
      return this.cachedMenuList
    }
    ...
    // 规范化路径（将相对路径转换为完整路径）
    const result = this.normalizeMenuPaths(menuList)

    // 写入缓存
    this.cachedMenuList = result
    this.cachedRolesSignature = rolesSignature
   ...
   */
  clearCache(): void {
    this.cachedMenuList = null
    this.cachedRolesSignature = ''
  }
