...
// 路由初始化完成的 Promise，用于让并发导航等待
let routeInitPromise: Promise<void> | null = null

// resetRouterState 的延迟定时器，用于在重新登录时取消未执行的注销
...
/**
 * 重置路由初始化状态（用于重新登录场景）
 * 同时取消任何待执行的路由注销操作，防止重新登录后路由被意外注销
 */
export function resetRouteInitState(): void {
  routeInitFailed = false
  routeInitInProgress = false
  routeInitPromise = null
  // 取消待执行的路由注销，防止重新登录后路由被意外移除
  if (resetRouterTimer) {
...
async function handleDynamicRoutes(
  ...
  // 标记初始化进行中，并创建 Promise 供并发导航等待
  routeInitInProgress = true
  routeInitPromise = doHandleDynamicRoutes(to, next, router)
  try {
    await routeInitPromise
  } finally {
    routeInitPromise = null
 ...
 * 实际执行动态路由注册逻辑
 */
async function doHandleDynamicRoutes(
  to: RouteLocationNormalized,
  next: NavigationGuardNext,
  router: Router
): Promise<void> {
  ...
  try {
    ...
    if (!hasPermission) {
      // 无权限访问，跳转到首页
      closeLoading()
      ElMessage.warning('当前账号没有访问该页面的权限，已为您跳转到可访问的首页')
      next({
        path: validatedPath,
        replace: true
      })
...
/**
 * 重置路由相关状态
 * 使用延迟注销以避免与导航竞态，定时器可在重新登录时被 resetRouteInitState 取消
 */
