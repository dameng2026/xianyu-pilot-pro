...
class VersionManager {
  ...
  private buildUpgradeMessage(requireReLogin: boolean): string {
    const currentRelease =
      releaseNotes.find(
        (item) => this.normalizeVersion(item.version) === this.normalizeVersion(StorageConfig.CURRENT_VERSION)
      ) || releaseNotes[0]
    const content = currentRelease?.title || '系统已完成安全与稳定性升级'

    const messageParts = [
      `系统已升级到 ${StorageConfig.CURRENT_VERSION} 版本。`,
      content
    ]

    if (requireReLogin) {
      messageParts.push(
        '为使安全策略立即生效，请重新登录后继续使用。'
      )
    }
