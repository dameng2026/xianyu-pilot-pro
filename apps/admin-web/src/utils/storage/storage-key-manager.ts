...
export class StorageKeyManager {
  /** localStorage 全量键缓存，避免每个 store 初始化时都执行 Object.keys(localStorage) */
  private static cachedStorageKeys: string[] | null = null
  ...
  /**
   * 查找其他版本的同名存储键
   * 使用缓存的键列表避免重复全量遍历 localStorage
   */
  private findExistingKey(storeId: string): string | null {
    // 首次调用时缓存 localStorage 的键列表
    if (StorageKeyManager.cachedStorageKeys === null) {
      StorageKeyManager.cachedStorageKeys = Object.keys(localStorage)
    }
    const storageKeys = StorageKeyManager.cachedStorageKeys
