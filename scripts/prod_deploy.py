...
def deploy_backend(
    ...
    bundle_path = create_backend_bundle()
    if verify_local_source:
        _assert_release_worktree_unchanged(release_key)
    ...
    with RemoteHost("china-backend", backend, dry_run) as remote:
        remote.upload(bundle_path, remote_bundle)
        extract_cmd = (
            "bash -lc "
            + shlex.quote(
                f"set -euo pipefail; "
                ...
                f"actual_sha=$(sha256sum {shlex.quote(remote_bundle)} | awk '{{print $1}}'); "
                f"if [ \"$actual_sha\" != {shlex.quote(bundle_sha256)} ]; then "
                "echo '[deploy] uploaded backend bundle checksum mismatch' >&2; exit 1; fi; "
                f"mkdir -p {shlex.quote(staged_project_dir)}; "
                f"tar -xzf {shlex.quote(remote_bundle)} -C {shlex.quote(staged_project_dir)}; "
                f"rm -f {shlex.quote(remote_bundle)}"
            )
        )
        remote.run(extract_cmd)
...
def deploy_frontend(
    ...
    user_bundle = create_dist_bundle("user-web", REPO_ROOT / "apps" / "user-web" / "dist", dry_run=dry_run)
    admin_bundle = create_dist_bundle("admin-web", REPO_ROOT / "apps" / "admin-web" / "dist", dry_run=dry_run)
    if verify_local_source:
        _assert_release_worktree_unchanged(release_key)
    remote_user_bundle = f"/tmp/{user_bundle.name}"
    remote_admin_bundle = f"/tmp/{admin_bundle.name}"
    ...
    with RemoteHost("us-frontend", frontend, dry_run) as remote:
        ...
        remote.upload(user_bundle, remote_user_bundle)
        remote.upload(admin_bundle, remote_admin_bundle)
        remote.upload(nginx_config_path, remote_nginx_bundle)

        deploy_script = f"""
set -Eeuo pipefail
mkdir -p {shlex.quote(backup_root)} {shlex.quote(release_root)}
USER_STAGE={shlex.quote(release_root + '/user-web-' + timestamp)}
ADMIN_STAGE={shlex.quote(release_root + '/admin-web-' + timestamp)}
...
NGINX_BACKUP={shlex.quote(f"{nginx_site_path}.bak-{timestamp}")}
USER_BACKED_UP=0
USER_INSTALLED=0
ADMIN_BACKED_UP=0
ADMIN_INSTALLED=0
...
rollback() {{
  ...
  rm -f "$NGINX_STAGE" {shlex.quote(remote_user_bundle)} {shlex.quote(remote_admin_bundle)} || rollback_failed=1
...
for candidate in "$USER_STAGE" "$ADMIN_STAGE" "$USER_BACKUP" "$ADMIN_BACKUP" "$ROLLBACK_STATE" {shlex.quote(user_failed)} {shlex.quote(admin_failed)}; do
  if [ -e "$candidate" ] || [ -L "$candidate" ]; then
    echo "[deploy] frontend release path collision" >&2
    exit 1
  fi
done
mkdir -m 700 "$ROLLBACK_STATE"
trap rollback ERR
mkdir -p "$USER_STAGE" "$ADMIN_STAGE"
tar -xzf {shlex.quote(remote_user_bundle)} -C "$USER_STAGE"
tar -xzf {shlex.quote(remote_admin_bundle)} -C "$ADMIN_STAGE"
rm -f {shlex.quote(remote_user_bundle)} {shlex.quote(remote_admin_bundle)}
...
"""
        try:
            _stdout, _stderr, deploy_exit_code = remote.run(
                "bash -lc " + shlex.quote(deploy_script),
                check=False,
                timeout=3600,
            )
        except Exception as exc:
...
NGINX_SITE={shlex.quote(nginx_site_path)}
NGINX_BACKUP={shlex.quote(f"{nginx_site_path}.bak-{timestamp}")}
NGINX_FAILED={shlex.quote(f"{nginx_site_path}.failed-{timestamp}")}
...
def main():
    args = parse_args()
    if not args.dry_run and (args.skip_frontend_build or args.skip_smoke):
        raise ProductionPreflightError(
            "--skip-frontend-build and --skip-smoke are permitted only with --dry-run"
        )
