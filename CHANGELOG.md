...
#### 移动端设计令牌系统化重构
- 19 个移动端页面 + 4 个移动端通用组件统一迁移至 CSS 设计令牌系统（`var(--m-color-*` / `var(--m-space-*` / `var(--m-radius-*` / `var(--m-shadow-*`）
...
- 涉及组件：MobileCategoryPicker（分类选择器）、MobileLocationPicker（地区选择器）、MobilePaymentModal（支付弹窗）、MobileOrderShipForm（订单发货表单）
